...
[spring-cli/samples]({{site.baseurl}}{{site.baseurl}}spring-cli/samples/#). To run the CLI samples type 
`spr run <sample>.groovy` from samples directory.

Java samples are available in [spring-zero-sample]({{site.baseurl}}{{site.baseurl}}spring-zero-samples/#) and should
